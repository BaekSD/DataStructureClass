...
public class Heap extends CBTree {
	...
	public void upBubbling(Node newNode) {
	...
	private void downBubbling(Node newNode) {
		...
		while(!tempParent.isLeaf()) {
			if(tempParent.getLeftChild() != null && tempParent.getRightChild() != null) {
				if(((Entry)tempParent.getLeftChild().getElement()).getKey() > ((Entry)tempParent.getRightChild().getElement()).getKey()) {
